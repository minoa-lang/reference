...
For more info, see [comparison operators](./operators/core-operators.md#comparison-operators-)
...
This trait indicates that a type which can be converted into an iterator to be able to be used in a [`for` expression](./expressions/loop-expressions.md#for-expression-).
...
This trait allows for a type to have a [`try` expression](./expressions/try-expressions.md).
...
This trait allows for a type to have a [`try!` expression](./expressions/try-expressions.md) be called on it.
...
It is used in tandem with [`CoerceUnsized`](#coerceunsized-)
...
This represents the desugared [nullable type](./type-system/types/opaque-types.md), which wraps another type and allows it to be `.None`

###  `Result` [↵](#types-)

The corresponding builtin name is `result`.

This represents the desugared [result type](./type-system/types/result-types.md), allowing a value to be valid or have an error value
...
This types stores the value created by a [key-path expression](./expressions/key-path-expressions.md).
...
It is also used to let the compiler know which tokenize meta-function to use for [meta-blocks](./metaprogramming/meta-utilities.md#meta-blocks-)
...
It is also used to let the compiler know which synthesize meta-function to use for [meta-blocks](./metaprogramming/meta-utilities.md#meta-blocks-)
